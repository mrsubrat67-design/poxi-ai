package com.poxi.mobile

import android.content.Context
import android.content.Intent
import android.hardware.camera2.CameraCharacteristics
import android.hardware.camera2.CameraManager
import android.media.AudioManager
import android.net.Uri
import android.os.BatteryManager
import android.provider.AlarmClock
import android.provider.Settings
import android.view.KeyEvent
import java.util.Calendar
import java.util.Locale

class PoxiCommandRouter(private val context: Context) {

    private val memory = PoxiMemory(context)

    fun handle(command: String): String {
        val original = command.trim()
        val c = original.lowercase(Locale.getDefault())

        return when {

            // MEMORY
            c.startsWith("remember my name is") -> {
                val name = original.substringAfter("remember my name is").trim()
                if (name.isEmpty()) {
                    "Please tell me your name."
                } else {
                    memory.saveName(name)
                    "Okay, I will remember your name is $name."
                }
            }

            c.contains("what is my name") ||
            c.contains("what's my name") ||
            c.contains("mera naam kya hai") ||
            c.contains("mera naam batao") ||
            c.contains("naam kya hai") -> {
                memory.getName()?.let {
                    "Your name is $it."
                } ?: "I don't know your name yet."
            }

            c.startsWith("remember that") ||
            c.startsWith("yaad rakho") ||
            c.startsWith("yaad rakhna") -> {

                val value = when {
                    c.startsWith("remember that") ->
                        original.substringAfter("remember that").trim()

                    c.startsWith("yaad rakho") ->
                        original.substringAfter("yaad rakho").trim()

                    else ->
                        original.substringAfter("yaad rakhna").trim()
                }

                if (value.isEmpty()) {
                    "What should I remember?"
                } else {
                    memory.saveFact(value)
                    "Okay, I will remember that."
                }
            }

            c.contains("what do you remember") ||
            c.contains("kya yaad hai") ||
            c.contains("maine kya yaad") -> {
                memory.getFact() ?: "I don't have anything remembered yet."
            }

            // BATTERY
            c.contains("battery") ||
            c.contains("बैटरी") ||
            c.contains("charge kitna") ||
            c.contains("charge kitni") -> {
                batteryStatus()
            }

            // SEARCH
            c.startsWith("search") ||
            c.startsWith("google") ||
            c.startsWith("internet pe search") ||
            c.startsWith("internet par search") ||
            c.startsWith("online search") -> {

                val query = extractSearchQuery(original)

                if (query.isEmpty()) {
                    "What should I search for?"
                } else {
                    webSearch(query)
                    "Searching for $query."
                }
            }

            // ALARM
            c.contains("set alarm") ||
            c.contains("alarm lagao") ||
            c.contains("alarm laga do") ||
            c.contains("alarm lagado") ||
            c.contains("alarm set") ||
            (c.contains("alarm") &&
                (c.contains("baje") ||
                 c.contains("am") ||
                 c.contains("pm"))) -> {
                setAlarmFromCommand(original)
            }

            // CAMERA
            c.contains("open camera") ||
            c.contains("camera kholo") ||
            c.contains("camera khol") ||
            c.contains("camera open") ||
            c.contains("camera chalu") ||
            c.contains("camera chalao") ||
            c.contains("camera on karo") -> {

                openCamera()
                "Opening camera."
            }

            // FLASHLIGHT
            c.contains("flashlight on") ||
            c.contains("torch on") ||
            c.contains("torch chalu") ||
            c.contains("torch jalao") ||
            c.contains("torch jala do") ||
            c.contains("torch on karo") ||
            c.contains("flashlight chalu") -> {

                if (setFlashlight(true)) {
                    "Flashlight on."
                } else {
                    "I couldn't turn on the flashlight."
                }
            }

            c.contains("flashlight off") ||
            c.contains("torch off") ||
            c.contains("torch band") ||
            c.contains("torch band karo") ||
            c.contains("torch bujhao") ||
            c.contains("torch bujha do") ||
            c.contains("flashlight band") -> {

                if (setFlashlight(false)) {
                    "Flashlight off."
                } else {
                    "I couldn't turn off the flashlight."
                }
            }

            // WIFI
            c.contains("wifi settings") ||
            c.contains("wifi kholo") ||
            c.contains("wifi khol") ||
            c.contains("wifi open") ||
            c.contains("wifi settings kholo") -> {

                openSettings(Settings.ACTION_WIFI_SETTINGS)
                "Opening Wi-Fi settings."
            }

            // BLUETOOTH
            c.contains("bluetooth settings") ||
            c.contains("bluetooth kholo") ||
            c.contains("bluetooth khol") ||
            c.contains("bluetooth open") -> {

                openSettings(Settings.ACTION_BLUETOOTH_SETTINGS)
                "Opening Bluetooth settings."
            }

            // PHONE SETTINGS
            c.contains("phone settings") ||
            c.contains("settings kholo") ||
            c.contains("settings khol") ||
            c == "settings" -> {

                openSettings(Settings.ACTION_SETTINGS)
                "Opening phone settings."
            }

            // APPS
            c.startsWith("open ") ||
            c.startsWith("launch ") ||
            c.startsWith("khol ") ||
            c.startsWith("kholna ") ||
            c.startsWith("khol do ") ||
            c.contains("youtube kholo") ||
            c.contains("youtube khol") ||
            c.contains("whatsapp kholo") ||
            c.contains("whatsapp khol") ||
            c.contains("instagram kholo") ||
            c.contains("instagram khol") ||
            c.contains("facebook kholo") ||
            c.contains("facebook khol") ||
            c.contains("spotify kholo") ||
            c.contains("spotify khol") -> {

                openAppByName(original)
            }

            // NOTES
            c.startsWith("save note") ||
            c.startsWith("note likho") ||
            c.startsWith("note likh") ||
            c.startsWith("note save karo") ||
            c.startsWith("save this") ||
            c.startsWith("isko note karo") -> {

                val note = when {
                    c.startsWith("save note") ->
                        original.substringAfter("save note").trim()

                    c.startsWith("note likho") ->
                        original.substringAfter("note likho").trim()

                    c.startsWith("note likh") ->
                        original.substringAfter("note likh").trim()

                    c.startsWith("note save karo") ->
                        original.substringAfter("note save karo").trim()

                    c.startsWith("save this") ->
                        original.substringAfter("save this").trim()

                    else ->
                        original.substringAfter("isko note karo").trim()
                }

                if (note.isEmpty()) {
                    "What note should I save?"
                } else {
                    memory.saveLastNote(note)
                    "Note saved."
                }
            }

            c.contains("last note") ||
            c.contains("meri note") ||
            c.contains("my note") ||
            c.contains("meri last note") ||
            c.contains("note kya hai") -> {
                memory.getLastNote() ?: "I don't have a saved note."
            }

            // TIME
            c.contains("what time") ||
            c.contains("time kya") ||
            c.contains("time batao") ||
            c.contains("kitne baje") ||
            c == "time" -> {

                val now = Calendar.getInstance()

                String.format(
                    Locale.getDefault(),
                    "The time is %02d:%02d.",
                    now.get(Calendar.HOUR_OF_DAY),
                    now.get(Calendar.MINUTE)
                )
            }

            // DATE
            c.contains("today's date") ||
            c.contains("what date") ||
            c.contains("date kya") ||
            c.contains("date batao") ||
            c.contains("aaj ki date") ||
            c == "date" -> {

                val now = Calendar.getInstance()

                String.format(
                    Locale.getDefault(),
                    "Today is %02d/%02d/%04d.",
                    now.get(Calendar.DAY_OF_MONTH),
                    now.get(Calendar.MONTH) + 1,
                    now.get(Calendar.YEAR)
                )
            }

            // WEATHER
            c.contains("weather") ||
            c.contains("mausam") ||
            c.contains("aaj ka mausam") -> {

                webSearch("weather today")
                "Opening today's weather information."
            }

            // MEDIA
            c.contains("pause music") ||
            c.contains("music pause") ||
            c.contains("pause media") ||
            c.contains("music roko") ||
            c.contains("music band") -> {

                mediaKey(KeyEvent.KEYCODE_MEDIA_PAUSE)
                "Pausing media."
            }

            c.contains("play music") ||
            c.contains("music play") ||
            c.contains("play media") ||
            c.contains("music chalao") ||
            c.contains("music chalu karo") ||
            c.contains("gaana chalao") ||
            c.contains("song chalao") -> {

                mediaKey(KeyEvent.KEYCODE_MEDIA_PLAY)
                "Playing media."
            }

            c.contains("next song") ||
            c.contains("next track") ||
            c.contains("agla gana") ||
            c.contains("agla song") ||
            c == "next" -> {

                mediaKey(KeyEvent.KEYCODE_MEDIA_NEXT)
                "Next track."
            }

            c.contains("previous song") ||
            c.contains("previous track") ||
            c.contains("pichla gana") ||
            c.contains("pichla song") ||
            c == "previous" -> {

                mediaKey(KeyEvent.KEYCODE_MEDIA_PREVIOUS)
                "Previous track."
            }

            // VOLUME
            c.contains("volume up") ||
            c.contains("volume badhao") ||
            c.contains("volume badao") ||
            c.contains("awaz badhao") ||
            c.contains("awaaz badhao") -> {

                adjustVolume(AudioManager.ADJUST_RAISE)
                "Volume increased."
            }

            c.contains("volume down") ||
            c.contains("volume kam") ||
            c.contains("awaz kam") ||
            c.contains("awaaz kam") -> {

                adjustVolume(AudioManager.ADJUST_LOWER)
                "Volume decreased."
            }

            c.contains("mute phone") ||
            c.contains("phone mute") ||
            c.contains("phone silent") -> {

                adjustVolume(AudioManager.ADJUST_MUTE)
                "Phone muted."
            }

            // PERMISSIONS
            c.contains("privacy") ||
            c.contains("permissions") ||
            c.contains("permission status") -> {

                openAppSettings()
                "Opening POXI app permissions."
            }

            // HELP
            c == "help" ||
            c.contains("what can you do") ||
            c.contains("tum kya kar sakte ho") ||
            c.contains("kya kya kar sakte ho") -> {

                "I can open apps, search the internet, check battery, open camera, control flashlight, set alarms, save notes, tell time and date, open Wi-Fi and Bluetooth settings, and control media."
            }

            else -> {
                "I don't know that command yet."
            }
        }
    }

    private fun batteryStatus(): String {

        val manager =
            context.getSystemService(
                Context.BATTERY_SERVICE
            ) as BatteryManager

        val level =
            manager.getIntProperty(
                BatteryManager.BATTERY_PROPERTY_CAPACITY
            )

        return if (level >= 0) {
            "Battery is at $level percent."
        } else {
            "I couldn't read the battery level."
        }
    }

    private fun extractSearchQuery(command: String): String {

        val lower =
            command.lowercase(Locale.getDefault())

        val prefixes = listOf(
            "internet pe search karo",
            "internet pe search",
            "internet par search karo",
            "internet par search",
            "online search",
            "search karo",
            "search kar",
            "google",
            "search"
        )

        for (prefix in prefixes) {
            if (lower.startsWith(prefix)) {
                return command.substring(prefix.length).trim()
            }
        }

        return command.trim()
    }

    private fun webSearch(query: String) {

        val url =
            "https://www.google.com/search?q=" +
                Uri.encode(query)

        val intent =
            Intent(
                Intent.ACTION_VIEW,
                Uri.parse(url)
            ).apply {
                addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
            }

        try {
            context.startActivity(intent)
        } catch (_: Exception) {
        }
    }

    private fun setAlarmFromCommand(
        command: String
    ): String {

        val regex =
            Regex(
                """(\d{1,2})(?::(\d{2}))?\s*(am|pm)?""",
                RegexOption.IGNORE_CASE
            )

        val match = regex.find(command)

        if (match == null) {
            return "Please say the alarm time, for example, set alarm at 7 AM."
        }

        var hour =
            match.groupValues[1].toInt()

        val minute =
            match.groupValues[2]
                .ifEmpty { "0" }
                .toInt()

        val ampm =
            match.groupValues[3]
                .lowercase(Locale.getDefault())

        if (ampm == "pm" && hour < 12) {
            hour += 12
        }

        if (ampm == "am" && hour == 12) {
            hour = 0
        }

        if (hour !in 0..23 || minute !in 0..59) {
            return "That doesn't look like a valid alarm time."
        }

        val intent =
            Intent(AlarmClock.ACTION_SET_ALARM).apply {

                putExtra(
                    AlarmClock.EXTRA_HOUR,
                    hour
                )

                putExtra(
                    AlarmClock.EXTRA_MINUTES,
                    minute
                )

                putExtra(
                    AlarmClock.EXTRA_MESSAGE,
                    "POXI Alarm"
                )

                addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
            }

        return try {
            context.startActivity(intent)
            "Opening alarm setup."
        } catch (_: Exception) {
            "I couldn't open the alarm app."
        }
    }

    private fun openCamera() {

        val intent =
            Intent("android.media.action.IMAGE_CAPTURE").apply {
                addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
            }

        try {
            context.startActivity(intent)
        } catch (_: Exception) {
        }
    }

    private fun setFlashlight(
        enabled: Boolean
    ): Boolean {

        val cameraManager =
            context.getSystemService(
                Context.CAMERA_SERVICE
            ) as CameraManager

        return try {

            val cameraId =
                cameraManager.cameraIdList.firstOrNull { id ->

                    val characteristics =
                        cameraManager.getCameraCharacteristics(id)

                    characteristics.get(
                        CameraCharacteristics.FLASH_INFO_AVAILABLE
                    ) == true
                }

            if (cameraId != null) {
                cameraManager.setTorchMode(
                    cameraId,
                    enabled
                )
                true
            } else {
                false
            }

        } catch (_: Exception) {
            false
        }
    }

    private fun openSettings(action: String) {

        val intent =
            Intent(action).apply {
                addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
            }

        try {
            context.startActivity(intent)
        } catch (_: Exception) {
        }
    }

    private fun openAppSettings() {

        val intent =
            Intent(
                Settings.ACTION_APPLICATION_DETAILS_SETTINGS,
                Uri.parse("package:${context.packageName}")
            ).apply {
                addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
            }

        try {
            context.startActivity(intent)
        } catch (_: Exception) {
        }
    }

    private fun openAppByName(
        command: String
    ): String {

        var name =
            command
                .replace(
                    Regex(
                        "^(open|launch|khol|kholna|khol do)\\s+",
                        RegexOption.IGNORE_CASE
                    ),
                    ""
                )
                .trim()

        name =
            name.replace(
                Regex(
                    "\\s+(khol|kholo|open|chalu karo|chalao|khol do)$",
                    RegexOption.IGNORE_CASE
                ),
                ""
            ).trim()

        if (name.isEmpty()) {
            return "Which app should I open?"
        }

        val pm = context.packageManager

        val launcherIntent =
            Intent(
                Intent.ACTION_MAIN,
                null
            ).apply {
                addCategory(Intent.CATEGORY_LAUNCHER)
            }

        val apps =
            pm.queryIntentActivities(
                launcherIntent,
                0
            )

        val app =
            apps.firstOrNull { info ->

                val label =
                    info.loadLabel(pm)
                        .toString()
                        .lowercase(Locale.getDefault())

                label.contains(
                    name.lowercase(Locale.getDefault())
                )
            }

        if (app == null) {
            return "I couldn't find $name."
        }

        val launchIntent =
            pm.getLaunchIntentForPackage(
                app.activityInfo.packageName
            )

        if (launchIntent == null) {
            return "I can't open $name."
        }

        launchIntent.addFlags(
            Intent.FLAG_ACTIVITY_NEW_TASK
        )

        return try {

            context.startActivity(launchIntent)

            "Opening ${app.loadLabel(pm)}."

        } catch (_: Exception) {

            "I couldn't open $name."
        }
    }

    private fun mediaKey(
        keyCode: Int
    ) {

        val audioManager =
            context.getSystemService(
                Context.AUDIO_SERVICE
            ) as AudioManager

        val down =
            KeyEvent(
                KeyEvent.ACTION_DOWN,
                keyCode
            )

        val up =
            KeyEvent(
                KeyEvent.ACTION_UP,
                keyCode
            )

        try {
            audioManager.dispatchMediaKeyEvent(down)
            audioManager.dispatchMediaKeyEvent(up)
        } catch (_: Exception) {
        }
    }

    private fun adjustVolume(
        direction: Int
    ) {

        val audioManager =
            context.getSystemService(
                Context.AUDIO_SERVICE
            ) as AudioManager

        try {
            audioManager.adjustVolume(
                direction,
                AudioManager.FLAG_SHOW_UI
            )
        } catch (_: Exception) {
        }
    }
}
